...
cd back-end
go test -v            # 全ケース
go test -run TestAuth -v   # 特定グループのみ
```
...
| 領域 | 理由 / メモ |
|---|---|
| 大量タスク（畑 25 マス超） | スロットが埋まったときに `field_position` が NULL のままになる挙動。フロントの 25 件上限（`Layout.tsx`）と合わせて確認 |
| フロントエンド | 本スイートは API のみ。画面操作（配置のドラッグ、収穫演出、枯死ポップアップ）は未カバー |
| リフレッシュトークンでの再発行 | トークン更新エンドポイントは未実装 |
